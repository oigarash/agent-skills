...
**Symptom:**
```
Error: Failed to create notebook: [SSL: CERTIFICATE_VERIFY_FAILED] certificate verify failed: unable to get local issuer certificate (_ssl.c:1010)
```

**Cause:** Corporate VPN (e.g., Cisco AnyConnect) injects custom CA certificates that Python's default certificate store doesn't recognize.

**Solution:** Set `SSL_CERT_FILE` to the system OpenSSL certificate bundle:
```bash
# Find the system cert path
python3 -c "import ssl; print(ssl.get_default_verify_paths().openssl_cafile)"
# Typically: /opt/homebrew/etc/openssl@3/cert.pem

# Prefix all nlm commands with:
SSL_CERT_FILE=/opt/homebrew/etc/openssl@3/cert.pem nlm <command>

# Or export for the session:
export SSL_CERT_FILE=/opt/homebrew/etc/openssl@3/cert.pem
```

**Note:** `REQUESTS_CA_BUNDLE` and `NODE_TLS_REJECT_UNAUTHORIZED=0` do NOT work for this issue. The `SSL_CERT_FILE` environment variable is required because nlm uses Python's `ssl` module directly.

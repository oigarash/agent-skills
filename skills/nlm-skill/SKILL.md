...
## Prompt Crafting & Content Generation Guide

When a user wants to create NotebookLM content (audio, video, slides, infographic), **always follow this interactive workflow**:

1. **Read [references/prompt-guide.md](references/prompt-guide.md)** — especially Section 2 (Human-Like Techniques) and Section 7 (Interactive Workflow)
2. **Phase 1 — Narrow down**: Use AskUserQuestion to establish artifact type, audience, purpose, and tone/style
3. **Phase 2 — Draft scenario**: Create a concrete outline (show structure for audio, slide outline for slides, storyboard for video, layout for infographic) and present it to the user
4. **Phase 3 — Get feedback**: Ask the user to review the draft and suggest changes
5. **Phase 4 — Generate**: Convert the approved draft into focus prompts + design prompts, then execute
6. **Phase 5 — Iterate**: After generation, offer revision options (slides revise, regeneration, etc.)

Key principles:
- **Separate content (`--focus`) from design (custom prompts/style flags)**
- **Eliminate AI-isms**: Include forbidden word lists in prompts to remove robotic patterns
- **Use personas**: Define speaker/author characters for more human-like output
- **Structure explicitly**: Specify segment structure (intro → sections → outro) rather than leaving it to AI defaults

## Advanced Reference

For detailed information, see:
- **[references/prompt-guide.md](references/prompt-guide.md)**: Prompt writing guide for all artifact types
